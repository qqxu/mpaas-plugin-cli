...
const { formatVersionForMPaaS } = require('@shuhejs/mpaas');
...
const MPAAS_CONFIG_PATH = 'config/mpaas';
...
const onDistDirectoryReady = () => {
  ...
  if (!exists) {
    console.log(`ERR: ${DIST_DIRECTORY_PATH} 不存在，请生成dist目录`);
    process.exit();
  }
}
...
const onPackageFileRead = () => {
  ...
  const { name, version, versionKey, mpaas = MPAAS_CONFIG_PATH, appId } = jsonConfig;
  
  const infoIsMiss = [name, version, versionKey, appId, mpaas].some(itm => !itm);
  if (infoIsMiss) {
    console.log(`ERR: 信息缺失，${PACKAGE_FILE_PATH} 中 name, version, versionKey, mpaas, appId 不可缺失`);
    process.exit();
  }
  return {
...
const updateMpaasFileSync = (mpaasPath, mPaasConfig, destPath) => {
  ...
  if (anyFileNonExit) {
    console.log(`ERR: 请检查${mpaasPath} 目录,需要包含 ${MPAAS_CONFIG_FILE_NAMES.join(' ')}，请注意大小写`);
    process.exit();
  }

  MPAAS_CONFIG_FILE_NAMES.forEach((fileName) => {
    const dataObj = fse.readJSONSync(`${PROCESS_PATH}/${mpaasPath}/${fileName}`, 'utf8'); // 可校验json格式 是否正确
...
const generateZip = () => {
  program
        ...
        .action(() => {
          ...
          const { name, version: originVersion, versionKey, appId, mpaas } = onPackageFileRead(); // 读取package.json 
         
          const version = formatVersionForMPaaS(originVersion);
